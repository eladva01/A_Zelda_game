...
def gaga2():    
    ...
    WBhit = [pygame.image.load('WBhit1.png'),pygame.image.load('WBhit2.png'),pygame.image.load('WBhit3.png'),pygame.image.load('WBhit4.png'),pygame.image.load('WBhit5.png')]
    
    clock=pygame.time.Clock()    
    h1= pygame.draw.rect(WIN , (255,0,0),   (80, 165,210,202),2) # house 1      
    h2= pygame.draw.rect(WIN , (255,0,0),   (535, 165,210,202),2) # house 2
    h3= pygame.draw.rect(WIN , (255,0,0),   (120, 495,290,202),2) #house 3
    ...
    #rect = pygame.draw.rect(WIN,(0,0,0),(man.DMUTx, man.DMUTy, man.DMUTh, man.DMUTw),2) #player
    ...
    def redrawGAME():
        ...
        pygame.draw.rect(WIN,(225,0,0),(jelly.x-8, jelly.y+15, jelly.width+18, jelly.height-5),2) #jelly
    # pygame.draw.rect(WIN, (255, 0, 0), (obs.x, obs.y, obs.w, obs.h),2) #table
        ...
        #man.draw(WIN)
        #pygame.draw.rect(WIN,(2,200,100),(200,350,50,50)) #level1 door
        #pygame.draw.rect(WIN,(2,200,100),(570,350,50,50))#kitchen door
        ...
        pygame.draw.rect(WIN , (0,0,255), (jelly.x, jelly.y -20, 50-(5* (10-jelly.health)), 10)) #mob health
    ...
    man = player(200,345,45,60)
    blade =sword(300, 410 ,64,64)
    jelly = enemy(400,200,50,50)
    ...
    while run:
        ...
        if Keys[pygame.K_UP] and man.DMUTy >25 :
                            if man.VEL > 0:
                                man.DMUTy -=man.VEL
                                man.down = False
                                man.up = True
        ...
        if Keys [pygame.K_LEFT] and man.DMUTx> 10: 
                        man.DMUTx -=man.VEL
                        man.standing = True
                        man.standing2 = False
                        man.down = False
        ...
        if Keys [pygame.K_RIGHT] and man.DMUTx < 770 :
                            man.DMUTx += man.VEL
                            man.standing = False
                            man.standing2 = True
                            man.down = False
        ...
        if Keys [pygame.K_DOWN] and man.DMUTy < 750 :
                            man.DMUTy +=man.VEL
                            man.down = True
                            man.up = False
                            man.left = False
        ...
        if jelly.x < man.DMUTx:
                jelly.x += 0.5
        if jelly.y < man.DMUTy:
                jelly.y += 0.5
        if jelly.x > man.DMUTx:
                jelly.x -= 0.5
        if jelly.y > man.DMUTy:
            ...
            # Check for collision
        Prect = pygame.Rect(man.DMUTx, man.DMUTy, man.DMUTw, man.DMUTh)
        Jrect = pygame.Rect(jelly.x-8, jelly.y+15, jelly.width+18, jelly.height-5)
        HITrect = pygame.Rect(blade.hitbox)

            # Check for collision
        collision_tolerence = 10
        if Prect.colliderect(Jrect):
            jelly.hit()
            man.hit()
            
            if abs(Prect.bottom - Jrect.top) < collision_tolerence:
                    man.DMUTy -= 4  
                    jelly.y += jelly.width              
                    #print('noice')
            elif abs(Jrect.bottom - Prect.top) < collision_tolerence:
                    man.DMUTy += 4    
                    jelly.y -= jelly.width                       
                # print('noice')
            elif abs(Prect.left - Jrect.right) < collision_tolerence:
                    man.DMUTx += 4
                    jelly.x -= jelly.height              
                # print('noice')
            elif abs(Prect.right - Jrect.left) < collision_tolerence:
                    man.DMUTx -= 4 
                    jelly.x += jelly.height                  
                # print('noice')
                
        if HITrect.colliderect(Jrect):
                print('collide')
                jelly.hit()

                if abs(Jrect.bottom - HITrect.top) < collision_tolerence:
                        man.DMUTy -= 4  
                        jelly.y -= jelly.width              
                        #print('noice')
                elif abs(HITrect.bottom - Jrect.top) < collision_tolerence:
                        man.DMUTy += 4    
                        jelly.y += jelly.width                       
                    # print('noice')   
                elif abs(HITrect.left - Jrect.right) < collision_tolerence:
                        man.DMUTx += 4
                        jelly.x -= jelly.height              
                    # print('noice')
                elif abs(HITrect.right - Jrect.left) < collision_tolerence:
                        man.DMUTx -= 4 
                        jelly.x += jelly.height                  
                    # print('noice')
                

        if Keys[pygame.K_SPACE]: # back to level1
                
                if man.DMUTx > 150 and man.DMUTx < 250 and man.DMUTy > 300 and man.DMUTy < 350 :
                    run == False
                    print('first door') 
                    time.sleep(1)
                    from level1 import gaga
                    gaga()

        if Keys[pygame.K_SPACE]: #to the kitchen
                if man.DMUTx < 630 and man.DMUTx >530 and man.DMUTy < 350 and man.DMUTy > 300 :
                    run == False
                    print('kitchen door')
                    time.sleep(1)
                    from kitchen import gagakit
                    gagakit()
                    

        if Keys[pygame.K_SPACE]: # to the bedroom
                if man.DMUTx > 250 and man.DMUTx < 320 and man.DMUTy > 650 and man.DMUTy < 690 :
                    run == False
                    print('bedroom door') 
                    time.sleep(1)
                    from bedroom import gagabed
                    gagabed()
        
        if Keys[pygame.K_SPACE]: # to the oouts
                if man.DMUTx > 410 and man.DMUTx < 490 and  man.DMUTy < 45 :
                    run == False
                    print('outside area') 
                    time.sleep(1)
